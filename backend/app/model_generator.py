...
class FPNInception(nn.Module):
    ...
    def forward(self, x):
        ...
        map4 = nn.functional.upsample(self.head4(
            map4), scale_factor=8, mode="nearest")
        map3 = nn.functional.upsample(self.head3(
            map3), scale_factor=4, mode="nearest")
        map2 = nn.functional.upsample(self.head2(
            map2), scale_factor=2, mode="nearest")
        map1 = nn.functional.upsample(self.head1(
            map1), scale_factor=1, mode="nearest")
...
class FPN(nn.Module):
    ...
    def __init__(self, norm_layer, num_filters=256):
        ...
        Args:
          ...
          pretrained: use ImageNet pre-trained backbone model feature extractor
        """
    ...
    def forward(self, x):
        ...
        map2 = self.td2(F.pad(lateral2, pad, "reflect") +
